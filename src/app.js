...
class TitanBot extends Client {
  ...
  async start() {
    try {
      ...
      // VoiceStateUpdate esemény manuális beregisztrálása
      this.on('voiceStateUpdate', async (oldState, newState) => {
        try {
          const voiceHandler = await import('./events/voiceStateUpdate.js');
          const handlerFn = voiceHandler.default || voiceHandler;
          if (typeof handlerFn === 'function') {
            await handlerFn(this, oldState, newState);
          } else if (handlerFn && typeof handlerFn.execute === 'function') {
            await handlerFn.execute(oldState, newState);
          }
        } catch (err) {
          logger.error('Hiba a voiceStateUpdate betöltésekor/futtatásakor:', err);
        }
      });
